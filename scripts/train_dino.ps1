...
@'
Usage:
  .\scripts\train_dino.ps1 --config PATH [--python PATH]
  .\scripts\train_dino.ps1 --data-root PATH --output-dir PATH [options] [--python PATH]

What it does:
  - launches the unified ag-foundation CLI
  - trains a DINOv2-style continual pretraining run
...
Examples:
  .\scripts\train_dino.ps1 --config .\configs\train_dino.example.yaml
  .\scripts\train_dino.ps1 --config .\configs\pretraining_dino_smoke.yaml --resume
  .\scripts\train_dino.ps1 --data-root D:\data\ag.zip --output-dir .\runs\ag-dino --channels 4 --precision fp16 --model-name S
'@ | Write-Output
}
